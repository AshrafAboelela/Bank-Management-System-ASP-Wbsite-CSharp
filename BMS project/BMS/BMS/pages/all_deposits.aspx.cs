...
namespace BMS.pages
{
    public partial class all_deposits : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            // binding deposits gridview
            retriving.functions.openconn();
            string sql;
           // sql = "select * from deposits";
            sql = "select dep_no as [المبلغ بالارقام],dep_str as [المبلغ بالحروف],date as [التاريخ],cust_name as [إسم الساحب],emp_name as [اسم الموظف],cust_phone as [التليفون],bank_id as [رقم الحساب] from deposits";
            OleDbCommand cmd = new OleDbCommand(sql);
            cmd.Connection = retriving.con;
            OleDbDataAdapter adapt = new OleDbDataAdapter(cmd.CommandText, retriving.con);
